      ...
      ! period measurement (added): previous y, and the iteration of the
      ! first descending y-crossing, so a full orbit can be timed
      real*8, dimension(10)::yprev
      integer*8, dimension(10)::icross, ncross
      real*8 tunit
      !las inicializo
      L=0.d0
      En=0.d0
      ...
      !que es el mas largo
      ! seconds per unit of code time
      tunit=1.d0/(sqrt(G*1.989e30/(c*c*c)))
      do J=1,10
         yprev(J)=r(J,2)
         icross(J)=0
         ncross(J)=0
      ...
      !periodo -- time a FULL orbit between two successive descending
      !crossings of y=0. The planets do not all start on the +x axis, so the
      !original "half a period has passed the first time y<0" test was only
      !valid for Earth and Uranus. This version is start-angle independent.
      do u=2,10
      if(ncross(u).lt.2)then
      if (yprev(u).ge.0.d0 .and. r(u,2).lt.0.d0) then
      ncross(u)=ncross(u)+1
      if (ncross(u).eq.1) then
      icross(u)=I
      else
      write(17,*) u, (I-icross(u))*h*tunit/86400.d0
      endif
      endif
      endif
      yprev(u)=r(u,2)
