//realizar una funcion que ordene por edades el vector p[]
//con apellidos, nombre,fnacimiento, localidad y sueldo
//libro de problemas,ejer 10.3
#include<iostream>
using namespace std;
...
void cPersona::rellenar(void)
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        getline(cin,p[i].fnac.fnacimiento());
        cout<<"sueldo: ";
        getline(cin,p[i].sueldo);
    }
}

void cPersona::mostrar(void)
{
  int i,n;
  cFecha fnac;  cpersona p[100];
  cout<<"numero de personas: ";
  ...
  cout<<"personas ordenadas: "<<endl;
    for(i=0;i<n;i++)
    {
        ...
        cout<<p[i].localidad;
        cout<<"fecha nacimiento: ";
        cout<<p[i].fnac.dia<<" de "<<p[i].fnac.mes<<" de "<<p[i].fnac.anio;
        cout<<"sueldo: ";
        cout<<p[i].sueldo;
    }
